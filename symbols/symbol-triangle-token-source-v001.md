# AST-SYM-003 — Triangle Token

Status: Approved visual master
Version: v001
Context: Campaign One — Scene 01, The Gate at Dusk

## Authoritative master

The authoritative lossless PNG master is stored in Google Drive:

- Drive file ID: `1F_Yen3NvRgAFtJz3_ttoqlwxfJTPEBFj`
- File name: `AST-SYM-003 — Triangle Token — Master v001.png`
- Drive folder: `Aramyst / 05 — Assets / Symbols & Tokens`
- Master dimensions: 1254 × 1254 px
- Master MIME type: `image/png`
- Master SHA-256: `a6963796ac766ea9aca914655ba059eab7836a508490c689b5e7dc1a82f29c17`

## Repository preview derivative

- Export path: `exports/symbol-triangle-token-preview-v001.webp.b64`
- Decode format: WebP
- Decoded dimensions: 256 × 256 px
- Decoded SHA-256: `611510ae2ebf1ad81f42e2e7ca43ce4fe84f6784b03e84d4251c960d80353d2f`

The repository preview is a compact review/layout derivative. The Google Drive PNG above remains the authoritative visual master.
